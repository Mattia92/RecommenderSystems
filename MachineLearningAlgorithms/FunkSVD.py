...
def load_active_during_test():
    items_data = pd.read_csv("../DataSet/item_profile.csv",
                             delim_whitespace=True,
                             dtype={'item_id': int, 'active_during_test': str},
                             usecols=['item_id', 'active_during_test'])

    return set(items_data[items_data.active_during_test == '1']['item_id'].values)
...
def get_items_mapping():
    items_data = pd.read_csv("../DataSet/item_profile.csv",
                             ...
                             usecols=['item_id'])
    item_ids = set()
    for item_id in items_data['item_id']:
...
def get_users_mapping():
    users_data = pd.read_csv("../DataSet/user_profile.csv",
                             delim_whitespace=True,
                             dtype={'user_id': int},
                             usecols=['user_id'])
    users_ids = set()
...
######-----PreProcessing per preparare le strutture dati-----#####
