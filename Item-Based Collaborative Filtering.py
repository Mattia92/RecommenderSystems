...
def loadDataset():
    ...
    for line in open("DataSet/interactions.csv"):
        (user_id, item_id, interaction_type) = line.split(";")
        try:
            if int(interaction_type) > 0:
                prefs.setdefault(user_id, {})
                prefs[user_id][jobs[item_id]] = int(interaction_type)
        except ValueError:
            count += 1
            print "value error found! " + user_id + item_id + interaction_type
        except KeyError:
            count += 1
            print "key error found! " + user_id + " " + item_id
    return prefs
