...
for user, item, interaction in interactions.values:
    user_items_dictionary.setdefault(user, {})[item] = int(interaction)

# dict {item -> (list of {user -> interaction})}
for user, item, interaction in interactions.values:
    item_users_dictionary.setdefault(item, {})[user] = int(interaction)
...
for user in user_user_similarity_dictionary_num:
    ...
    for user2 in user_user_similarity_dictionary_num[user]:
        ...
        user_user_similarity_dictionary[user][user2] = user_user_similarity_dictionary_num[user][user2] / \
                                                      ((math.sqrt(user_user_similarity_dictionary_den1[user][user2]) *
                                                        math.sqrt(user_user_similarity_dictionary_den2[user][user2])) +
                                                       similarity_shrink)
